...
public class KinectPointController : MonoBehaviour {
	...
	void Start () {
		...
		log = new StreamWriter(@"C:\Users\eleve\Documents\JIN\IEC.b\TP Kinect\log.txt");
	...
	void Update () {
			...
			for( int ii = 0; ii < (int)Kinect.NuiSkeletonPositionIndex.Count; ii++) {
				...
				if( ((uint)Mask & (uint)(1 << ii) ) > 0 ){
					...
					_bones[ii].transform.localPosition = new Vector3(
						...
						if (logging) log.Write(Time.time + " " + sw.bonePos[player,ii].x * scale + " " + sw.bonePos[player,ii].y * scale + " " + sw.bonePos[player,ii].z * scale + " \n");
				}
			}
		}
		...
		{
			logging = (!recorded) ? !logging: logging ;
			if(!logging) 
			{
				log.Close();
				recorded = true;
